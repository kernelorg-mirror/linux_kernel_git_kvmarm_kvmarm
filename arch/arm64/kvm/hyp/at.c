...
void __kvm_at_s1e01(struct kvm_vcpu *vcpu, u32 op, u64 vaddr)
{
	...
	write_sysreg_el1(__vcpu_sys_reg(vcpu, TTBR0_EL1),	ttbr0);
	write_sysreg_el1(__vcpu_sys_reg(vcpu, TTBR1_EL1),	ttbr1);
	write_sysreg_el1(__vcpu_sys_reg(vcpu, TCR_EL1),		tcr);
	write_sysreg_el1(__vcpu_sys_reg(vcpu, SCTLR_EL1),	sctlr);
	 ...
	 */
	if (__vcpu_sys_reg(vcpu, PAR_EL1) & 1)
		goto nopan;
	...
	if (!(read_sysreg(par_el1) & 1))
		__vcpu_sys_reg(vcpu, PAR_EL1) = 0x1f;
...
void __kvm_at_s1e2(struct kvm_vcpu *vcpu, u32 op, u64 vaddr)
{
	...
	if (vcpu_el2_e2h_is_set(vcpu)) {
		write_sysreg_el1(__vcpu_sys_reg(vcpu, TTBR0_EL2),	ttbr0);
		write_sysreg_el1(__vcpu_sys_reg(vcpu, TTBR1_EL2),	ttbr1);
		write_sysreg_el1(__vcpu_sys_reg(vcpu, TCR_EL2),		tcr);
		write_sysreg_el1(__vcpu_sys_reg(vcpu, SCTLR_EL2),	sctlr);

		val = config.hcr;
	} else {
		write_sysreg_el1(__vcpu_sys_reg(vcpu, TTBR0_EL2),	ttbr0);
		write_sysreg_el1(translate_tcr(__vcpu_sys_reg(vcpu, TCR_EL2)),
				 tcr);
		write_sysreg_el1(translate_sctlr(__vcpu_sys_reg(vcpu, SCTLR_EL2)),
				 sctlr);
